 ...
 *
 * Everything is read-only in this milestone. Creating and editing events means a recurrence editor
 * and an RSVP flow, and a calendar that lets a user half-edit a recurring meeting is a calendar
 * that loses other people's time.
 */
...
import { type CalendarClient, makeCalendarClient, type PlacedEvent } from './calendar-client'
...
export default function CalendarPage(props: CalendarPageProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setEvents(await client.eventsInRange(new Date(fromMs), new Date(toMs)))
      setFailed(false)
    } catch {
      setFailed(true)
    }
  ...
  return (
    <div className={styles.page}>
      ...
      {events === null && !failed ? (
        ...
        <MonthView days={days} byDay={byDay} locale={locale} onPick={goto} />
      ) : (
...
function MonthView({ days, byDay, locale, onPick }: MonthViewProps) {
            ...
            >
              ...
              <span className={styles.dayEvents}>
                {dayEvents.slice(0, 3).map((placed) => (
                  <span key={placed.event.id} className={styles.chip}>
