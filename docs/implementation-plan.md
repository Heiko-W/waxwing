...
| **B53** | **Open (2026-08-23, observed while verifying M-13, NOT explained): three full `e2e:read` runs on the M-13 branch each failed exactly one test, and the three were DIFFERENT tests** — `read.spec.ts` "an attached .eml opens inline" (pos. 65), `read.spec.ts` "the keyboard alone reorders folders" (pos. 68) and `notify.spec.ts` "a live delivery raises a banner" (pos. 103, which is **B45**'s test with B45's exact signature). Each is green in isolation, and every failure is a TIMEOUT waiting for a sync/delivery — never a wrong assertion about content, which is what a data defect would look like. **Four hypotheses were tested and three were killed by measurement, not by argument.** (1) *The M-13 production change:* the branch with its two new tests removed (`--grep-invert M-13`, i.e. the same 106 tests as the baseline, same load, only the changed engine code) ran **106/106 green**. (2) *Suite size — two extra tests tip a fixture already at its limit:* unmodified `main` with two dummy tests appended that do the same work (login, open a folder, assert) ran **108/108 green**. (3) *The new tests polluting the fixture:* two of the three failures sit at positions 65 and 68, ahead of the M-13 tests at 72/73, so the fixture state at the point of failure is bit-identical to the green runs — a cause cannot act backwards. (4) *Chance:* a fourth branch run then came back **108/108 green**, breaking the 3-for-3 streak. Fisher's exact over the whole set (3 failures in 5 runs with the change, 0 in 3 without) gives **p = 0.196** — not significant. **So: nine full runs, ~55 minutes of wall clock, and the honest answer is that the difference is not established.** Recorded rather than closed, because "three in a row" deserves to be recognised if it recurs instead of being re-investigated from scratch. If it does recur, the next step is the one not taken here: instrument the JMAP request count per run and compare, which is the only mechanism by which a widened query could plausibly starve an unrelated test. | — | M5 (polish) | open |
