...
| WP | Title | Size | Depends on | Status | Notes |
|---|---|---|---|---|---|
| M4.0 | Web Push, contentless (FR-NOTIF-02 — decision D6a) | M | G2, M3.5, M3.6 | todo | Numbered `.0` so M4.1–M4.9 keep the numbers other documents already point at. Scope in [ADR-017](adr/017-web-push-contentless.md). |
...
- [ ] `@waxwing/jmap`: `PushSubscription` types + `PushSubscription/get|set` (RFC 8620 §7.2),
      and the `PushVerification` object. Keep the transport-agnostic shape of `push/` — this is
      a data type, not a fourth transport.
- [ ] Subscribe flow in the app (not the worker): `PushManager.subscribe()` with the server's
...
- [ ] `PushVerification` round-trip: the worker relays the verification push to the page,
...
- [ ] `push` listener in `apps/web/src/sw/sw.ts`: decrypt (the browser does this), confirm the
...
- [ ] Preferences the worker can honour, read from `localPrefs` (unencrypted IndexedDB):
...
- [ ] Renewal: the subscription expires after **7 days** and the ceiling is the server's
...
- [ ] Settings copy replaces `notify.background.notImplemented` with the truthful states,
      en + de. No key may claim more than the code does — the reason that string exists.
- [ ] Tests: the subscribe flow, the verification round-trip, the suppression rule, renewal
      and the `push` handler's frame filtering. The **closed-app delivery itself is
      unautomatable** (Playwright cannot observe a closed app) — verify by hand per platform
      and record what was checked, including whether iOS still requires Home-Screen install.

Done when: a real browser, app fully closed, raises one contentless banner on delivery to the
fixture and none on a message read elsewhere; opening it lands in Waxwing; the settings state
the 7-day limit and the per-folder gap; and no token or `SecretStore` call exists in `sw.ts`.
...
| **B22** | **Open (2026-07-22, G2 review): two test surfaces that look like coverage and are not.** (1) The `@waxwing/jmap` integration suites (`vitest.integration.config.ts`) run in NEITHER `pnpm verify` nor `pnpm verify:e2e`, and they fail OPEN when the fixture is down — so nothing has ever failed because they did not run. This project has been bitten by exactly this before: the E2E gate was red for eight milestones before M3.10 noticed. (2) `apps/web` resolves `@waxwing/mail-html` to built `dist/`, not `src/`, so a bare `pnpm vitest run` after editing that package silently tests the OLD library. It bit two waves of the G2 review. `pnpm verify` runs `build:libs` first, so the gate is sound; the hazard is every path that is not the gate, and nothing warns. | — | M4 (release engineering) | open |
