...
| **B46** | **Open (2026-08-18, seen ONCE in a full `pnpm test` run and NOT reproduced in three consecutive re-runs): `ContactsScreen.groups.test.tsx` "creates a group through the New-group form" could not find the heading "New group".** The accessible-roles dump in the failure shows the rail rendered normally ("Address books", "Groups") but no dialog at all, so the form had not mounted when the query ran rather than having mounted wrongly. The file passes on its own (7/7) and the whole suite passed three times in a row afterwards, so this is recorded rather than diagnosed — the same posture as B44 and B45, and for the same reason: a green re-run is evidence about the re-run, not about the failure. Worth a look if it recurs; the likely shape is a lazy dialog chunk resolving after the assertion under load rather than anything in the component. | — | M4.2 | open |
| **B47** | **Open (2026-08-18, seen ONCE in a full `verify:e2e` run and NOT reproduced): `narrow.spec.ts` "re-choosing the folder already open closes the drawer too" timed out waiting for a seeded subject.** The failure's own page snapshot is the useful part: the header, the folder title and the drawer had all rendered correctly, the grid held six SKELETON rows, and the status region read "Sync problem — retrying". So the layout was right and the envelopes never hydrated — the fixture, not the component. It is test 81 of 89, i.e. ~4 minutes into a run that has already driven Docker, a live Stalwart and an SSE stream, which is the same position and the same shape as B45. The phone project passes 8/8 on its own and the whole chain passed on the next run. Recorded rather than diagnosed: three sync-timing flakes deep in long runs (B44, B45, this) now point at the fixture's behaviour under sustained load rather than at three unrelated tests, and that is worth one investigation instead of three. | — | M4.7 | open |
| **B48** | **FIXED (2026-08-19, reported from the live deployment `mail.hcw-orange.media/webmail`): with the own mailbox expanded, the shared accounts below it could not be reached.** The owner's rail holds his own tree (five system folders plus eight named ones, one of them expanded) and three delegated accounts; the shared sections showed their headers and an "Ordner" row with nothing under them, and the only way down was to collapse the tree above first. **Cause: the scroll containers were in the wrong boxes.** Each `FolderTree` scrolled inside itself (`overflow-y: auto` on `.tree`) while the rail — `.folderRegion`, a fixed-height flex column — scrolled not at all, and every section was a shrinkable sibling (`.container { flex: 0 1 auto }`, `.accountSection` with `min-block-size: 0` and no basis) in it. So the tall tree took the height, the rest were squeezed toward their headers, and past that the column simply ran off the bottom with nothing to scroll. **Fix:** one scroll container for the rail (`.folderScroll`, wrapping the accounts and the labels; the storage bar stays outside it, pinned below), no `overflow-y` per tree, and every section sized to content (`flex: 0 0 auto`). **Why no suite saw it:** every fixture-backed spec runs at a viewport tall enough to hold all three accounts, and Playwright's `.click()` auto-scrolls the nearest scroller — which was precisely the box that should not have had one. The regression test (`shared.spec.ts`, "a shared account is reachable with the own tree expanded") pins both halves at a 300 px viewport: exactly ONE scrollable container in the rail and it is the rail's own child, and the LAST account's Inbox has real height and is clickable. Measured against the pre-fix CSS it reports **zero** scrollable containers while the content does not fit, which is the bug stated as a number. | M4.4 | M5 (polish) | fixed |
| **B49** | **Open (2026-08-19, found by photographing a tablet for the first time): the reading pane's action bar takes THREE rows in a narrow pane, and no amount of CSS will fix the last one.** At 834px the reading pane is under 300px wide and the bar holds eleven icon buttons — which on a touch device are **44px**, not 34px (`--waxwing-control-min` under `pointer: coarse`, WCAG 2.5.5 AAA). Eleven times 44 is 484px into 270px: two rows is the arithmetic best case, and the shipped layout managed three. Two thirds of the cause WAS fixable and is fixed — the rules that tighten the bar were keyed to the VIEWPORT (`max-width: 39.999em`), so they fired on a phone and stayed idle on a tablet whose pane is just as narrow; they are container queries now (`waxwing-pane` on `.paneBody`), and the conversation's inline padding gives way with them. That took it from three full rows to 5 + 5 + 1. **What remains needs a decision, not CSS:** the honest fix is Apple Mail's — keep four or five actions and move the rest into the `⋯` menu below a threshold. That menu currently holds only secondary items (snooze, forward-as-attachment, print, source), so hiding buttons with CSS would make real actions unreachable; the items have to be ADDED to the menu at the same width they leave the bar, which is a JS decision about which four actions are primary. | M1.8 | M5 (polish) | open |
| **B50** | **Open (2026-08-19, seen while photographing the settings rebuild): the compose button floats over every screen, including Settings.** On a phone the "New message" FAB sits above the bottom bar on the Settings list, the Contacts rail and the Calendar — surfaces where "new message" is not the obvious next action and where it covers content (in `phone-settings.webp` it sits on top of the last rail row). Gmail and Apple Mail both scope the equivalent to the mail area. Not fixed here because it is a scoping decision rather than a defect: the button is reachable from anywhere on purpose today, and `c` / ⌘N are global by design. Deciding it needs one sentence from the owner — mail-only, or everywhere but out of the content's way. | M2.2 | M5 (polish) | open |
