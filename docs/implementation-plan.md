...
| **B53** | **Open (2026-08-23, observed while verifying M-13; the CHANGE is exonerated, the flake class is not): across six full `e2e:read` runs on the M-13 branch, four failed exactly one test each** — `read.spec.ts` "an attached .eml opens inline" (pos. 65), `read.spec.ts` "the keyboard alone reorders folders" (pos. 68, **twice**) and `notify.spec.ts` "a live delivery raises a banner" (pos. 103, which is **B45**'s test with B45's exact signature). Three unmodified-`main` runs failed none. Every failure is a TIMEOUT waiting for a sync/delivery — never a wrong assertion about content, which is what a data defect would look like — and each test is green in isolation. **Five hypotheses, four killed by measurement rather than by argument.** (1) *The M-13 production change:* the branch with its two new tests removed (`--grep-invert M-13`, i.e. the same 106 tests as the baseline, same load, only the changed engine code) ran **106/106 green**. (2) *Suite size — two extra tests tip a fixture already at its limit:* unmodified `main` with two dummy tests appended doing the same work (login, open a folder, assert) ran **108/108 green**. (3) *The new tests polluting the fixture:* two failures sit at positions 65 and 68, ahead of the M-13 tests at 72/73, so the fixture state at the point of failure is bit-identical to the green runs — a cause cannot act backwards. (4) *The change widening the query starves the fixture:* **JMAP traffic was instrumented and compared on both branches for the same login + folder walk — 44 requests, identical method-call counts (`Email/query` 6, `Email/get` 12, `Email/queryChanges` 3, …), and 333 bytes FEWER with the fix**, because the dropped `after` term shortens every filter. There is no extra load to starve anything with. (5) *Chance:* Fisher's exact over the set (4 failures in 6 runs with the change, 0 in 3 without) gives **p = 0.167** — not significant, though the same test failing twice is more than the p-value alone suggests. **What the evidence DOES support, measured:** the reorder test takes **1.7 s in isolation (6/6 green on `main` at `--repeat-each=6`) and 14.1 s inside a full run — a factor of 8 — against a 30 s poll timeout**, and its failing run took 33.2 s. The suite carries tests that run near their timeout under sustained load, and which one tips is arbitrary; the fixture throttles deliberately (`e2e/stalwart/http.mjs`) while the app under test gets no retry concession. **Next step if it recurs — and it is about the SUITE, not this change:** raise the poll budgets on the load-sensitive tests, or give the suite headroom, rather than re-investigating the change each time. Ten full runs and ~70 minutes went into this; recorded so nobody repeats them. | — | M5 (polish) | open |
