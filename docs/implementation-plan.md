...
- [ ] **Handed over by the G2 gap package (2026-07-20).** Three things it built are unverifiable
      ...
      - **B4:** the two WebSocket 401s must be **gone from the console** and the SSE request
        must be the first push request at login. Measured before the fix at WS 85 ms → 401,
        WS 584 ms → 401, SSE 586 ms; assert the negative (no WS attempt), since that is what
        regressed silently for a milestone.
      - **B1:** with "Unread first" on, marking a message read must re-sort it **without
        waiting for the server's echo**.
...
| **B7** | **Open (surfaced by B1+B2, 2026-07-20; pre-existing): folder counts go stale where the list no longer does.** `Mailbox.unreadEmails` / `totalEmails` are server-owned and are not patched optimistically. Until now that was invisible, because the *list* was equally stale; B1 and B2 made the list update instantly, so the row now moves while the folder-tree badge beside it does not, until the server's delta lands. Offline that gap lasts until reconnect. This is a genuine local-computation question (an unread count can be derived from the replica only for the messages the replica holds), not a copy of the window fix. | — | Gate G2 | open |
| **B8** | **Open (measured by B2, 2026-07-20; pre-existing, not a regression): a BULK move places none of its arrivals if the window already lists any one of them.** `updateWindows` splits on membership with `window.ids.some(id => touched.has(id))` — one id present marks the whole window "listed", so a move of `[e1,e2]` into a window already showing `e1` places neither and does not even void. Observed directly (`ids: ["a1","e1"]`, `queryState: "q-1"`). The guard has had this shape since M3.8, so B2 did not introduce it — but B2 raised the cost from a delayed re-query to a **visibly missing row**. The fix is per-id membership, which is a change to B1's three-way split and its tests, and was deliberately not attempted late in the same work package. | — | Gate G2 | open |
| **B9** | **Open (carried from M3.9, 2026-07-19): the bulk bar's FLAG button is hard-wired to `setFlagged(ids, true)` while the `s` chord toggles.** The same click-vs-keystroke drift M3.9 closed for read/unread, one control over — two entry points to one user intent that do not agree. No swipe direction maps to flag, so it was out of scope then; it is a two-line fix via `triage.setFlagged`. Worth doing together with **B8**, since both are "the bulk path behaves unlike the single path". | — | Gate G2 | open |
