...
| **B44** | **Open (2026-08-17, seen ONCE on a hosted CI runner; investigated 2026-08-20 and the recorded hypothesis was DISPROVED): `MessageList.test.tsx` "a direction set to Archive is INERT on an account with no Archive" failed with `data-swipe === 'left'`.** The original note suspected the gate's negative half — that "no Archive layer" is also satisfied by a render where `useMailboxes()` has not settled. **Measured, that cannot happen here.** Two findings, either of which is sufficient: (1) the strip and the decision are the SAME call — `<SwipeLayer resolved={resolveSwipe(id, 'left')} />` — so "no Archive strip in this render" means `resolveSwipe` returned null in that render, and `data-swipe` is written only where it returned non-null; (2) a probe over `useLiveQuery` recorded the value sequence `[undefined, 'archive,inbox,trash', 'inbox,trash']` across exactly this delete — after the first resolution it never returns to `undefined`, and the test waits for the Archive layer BEFORE deleting, which pins it. The gate is airtight, and it has been since 2026-07-19 (the layer wait) and 2026-07-25 (the hydration conjunction) — both predate the sighting. Six further runs under CPU load did not reproduce it, on top of the 11 in the original report. **So the cause is unknown, and the leading explanation remains the environment the original note already named**: the failing run coincided with a GitHub incident (repeated 429/502/503 from codeload and the API) on a starved 2-core runner. Left open deliberately rather than closed on a guess or patched on a hypothesis now known to be wrong. **Two further sightings on 2026-08-20 (the second UI round), both LOCAL — the first time outside CI.** Neither is usable as evidence: both fell in `pnpm test` runs during which source files were being edited in the same working tree, so vitest was reading a moving target. Five clean full runs afterwards, with nothing else touching the tree, were green. That is one more thing this flake is not, and no closer to what it is. One real weakness WAS fixed while looking: the gate used `queryByText`, which THROWS on multiple matches — and inside `waitFor` a throw reads as "not settled", so the exact failure shape it guards against (a strip on every row) would have been hidden behind a timeout on the wrong thing. It is `queryAllByText` now, as the assertions below it already were. | M3.9 | — | open |
...
| **B52** | **Open (2026-08-21, seen ONCE in a full `pnpm verify:e2e`, not reproduced since): `narrow.spec.ts` "the settings list scrolls to its last section, and that section opens" timed out on its FIRST line — `getByRole('heading', { name: 'Settings', level: 1 })` never appeared after clicking the Settings link, and the failure snapshot shows the app still on Mail (`heading "Inbox"`, the message grid, the compose button).** The link itself is in that snapshot with the right target (`/settings`), and Playwright raises on an unhittable target, so the click landed — the route simply did not change within 5 s. **Not reproduced**: the same test at the same viewport passed 39/39 across three repeats immediately afterwards, and the next full `verify:e2e` was green end to end. Two candidates, neither demonstrated: the lazy `SettingsPage` chunk stalling under the load of a full suite run, or a click arriving before the router had taken over the link. **What it is NOT:** a port clash with the dev server left running from a manual session — that listens on 5173 while the read suite builds and previews on 4183, and the two never meet. Deliberately not "fixed" by raising the timeout: that would remove the symptom and leave the question. Left open with its evidence, like B44. | — | M5 (polish) | open |
