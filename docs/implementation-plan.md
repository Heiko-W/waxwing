...
- [ ] Read/write per server-granted rights — **not done, and stage 4 unmasked it (B34):**
      message-level writes check no `myRights` at all and `isReadOnly` is a badge only, so
      triage is fully enabled in a read-only shared mailbox. Recommended next stage.
...
- [ ] **E2E delegation fixture + `e2e/tests/shared.spec.ts`.** No coverage exists and none is
      possible: `e2e/stalwart/fixture.mjs` provisions three standalone users and no
      delegation, so `secondaryMailAccounts()` returns `[]` and every M4.4 path is dead in
      every suite. Probe a live Stalwart first (principal membership via the management API
      vs. RFC 9670 `Mailbox/set` `shareWith`), with ONE acceptance criterion checked before
      any code: alice's `/.well-known/jmap` must list the shared account with
      `isPersonal:false` AND `urn:ietf:params:jmap:mail` in that account's OWN
      `accountCapabilities` — that per-account URN is what `packages/jmap/src/session.ts`
      filters on. Trap: `up` never wipes the volume, so run `pnpm e2e:server:down` once
      before the first `up` with the new delegation step.
