 ...
 * locale and back.
 *
 * ── Why this file is called `account-security` and not `security` (R-44) ─────────────────────
 * It used to be `security.spec.ts`. The B25 experiment reused that name and OVERWROTE these five
 * tests instead of moving them: for a while the write config still listed `security.spec.ts` with a
 * comment describing "Account & security (X-1..X-6)", but the file behind the name had become three
 * read-harness browser experiments — so the app-password path lost its only live coverage, and the
 * B25 file ran twice per gate (once in each config) for no gain. Restored here under a name of its
 * own from `81ff67f`, the last commit that maintained it. `security-suite.source.test.ts` in
 * apps/web fails if any spec is listed by two gate configs again.
 */
